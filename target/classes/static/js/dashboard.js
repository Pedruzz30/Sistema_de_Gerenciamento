// Dashboard data loading and permission-based section visibility.
...
async function carregarDados() {
  ...
  let movimentacoes = [];
  let metricas = null;
  const podeVerEstoque = temPermissao(PERMISSOES.VER_ESTOQUE);
  const podeVerFinanceiro = temPermissao(PERMISSOES.VER_VENDAS);
...
function renderSummaryCards(produtos, caixas) {
  ...
  var cards = [
    ...
    },
    {
      ...
      trendText: alertas > 0 ? 'precisam de atencao' : 'tudo em ordem',
      perm: PERMISSOES.VER_ESTOQUE,
      href: '/estoque'
    },
    {
  ...
  grid.innerHTML = cards
    ...
    .map(function (c, i) {
      var trendIcon = c.trend === 'up' ? '^' : c.trend === 'down' ? 'v' : '-';
...
function renderPermissionSections() {
  ...
  if (temPermissao(PERMISSOES.VER_VENDAS)) {
...
function renderAlertas(produtos) {
  ...
  if (alertas.length === 0) {
    list.innerHTML = '<div class="empty">Todos os produtos estao em nivel adequado.</div>';
    return;
  }

  var nivelClass = { CRITICO: 'critico', BAIXO: 'baixo', MODERADO: 'moderado' };
  var nivelLabel = { CRITICO: 'Critico', BAIXO: 'Baixo', MODERADO: 'Moderado' };
...
function renderMovimentacoes(movimentacoes) {
  ...
  if (!movimentacoes || movimentacoes.length === 0) {
    list.innerHTML =
      ...
      '<div style="color:var(--text-3);font-size:13px">Nenhuma movimentacao registrada ainda.</div>' +
      '</div>';
    return;
  }
  ...
  list.innerHTML = movimentacoes.slice(0, 10).map(function (m) {
    ...
    try {
      ...
      if (tIdx !== -1) {
        hora = s.substring(tIdx + 1, tIdx + 6);
      } else {
...
function renderFinanceiro(caixas, metricas) {
  if (!temPermissao(PERMISSOES.VER_VENDAS)) return;
